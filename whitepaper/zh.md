...
4. 數位資產價值映射：TTC 發行 210,000,000,000 TTT 作為區塊鏈原生代幣，作為保管於 TTC 的數位資產價值轉換單位。
5. 分權均衡激勵原則：對等節點網路技術在資訊傳播技術具有很大的效能瓶頸，TTC 在節點的共識取消過往區塊鏈常用的算力證明機制（Proof of Work）採用分權制衡（Checks and Balances, CAB）機制，共識過程中所有節點相互監督，藉由獎勵機制鼓勵節點參與共識，藉由懲罰機制阻止節點作惡。該機制節省算力競爭帶來的資源浪費，並帶來更高的交易速度與頻寬。
# 原生代幣發行準則
...
# 跨鏈通訊協議
自 2009 年 Bitcoin 公開至今，已有上千種區塊鏈技術組成今天的區塊鏈生態。然而區塊鏈應用在開發或推廣上仍存在以下問題，在生態中形成龐大的成本，因此我們急需可靠的跨鏈通訊協議來解決這個問題。
1. 區塊鏈與區塊鏈之間溝通成本過高
2. 交易頻寬過低：以 Bitcoin、Ethereum 為例，區塊鏈 TPS 約為 7-25，造成其作為一般服務應用的用戶體驗極差。
3. 手續費問題：由於每一筆區塊鏈的交易都需要付費給礦工，意即用戶在區塊鏈上使用支付服務、上傳一張照片時，都需要付出不少的成本，這造成極高的用戶進入門檻。
## BOLT: Booster of Ledger Technology
BOLT 作為 Tidetime Chain 原生跨鏈通訊協議，可以安全地將其他區塊鏈上的資產或應用，安全地移轉到 Tidetime Chain 上，加速其運行，並產生更多服務整合的可能性。
